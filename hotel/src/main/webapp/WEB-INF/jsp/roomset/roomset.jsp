            ...
            <c:forEach items="${list.result}" var="item">
                <tr>
                    ...
                    <c:if test="${item.roomStateID==1}">
                        <td style="background:rgba(19,226,40,0.61);">${item.roomName}</td>
                    </c:if>
                    <c:if test="${item.roomStateID==2}">
                        <td style="background:#cbcb20;">${item.roomName}</td>
                    </c:if>
                    <c:if test="${item.roomStateID==4}">
                        <td style="background:#f59506;">${item.roomName}</td>
                    </c:if>
                    <c:if test="${item.roomStateID==5}">
                        <td style="background:rgba(7,115,100,0.25);">${item.roomName}</td>
                    </c:if>
                    <c:if test="${item.roomStateID==6}">
                        <td style="background:#b7068f;">${item.roomName}</td>
                    </c:if>
                    <c:if test="${item.roomStateID==7}">
                        <td style="background:#07e0f5;">${item.roomName}</td>
                    </c:if>
                    <c:if test="${item.roomStateID==65}">
                        <td style="background:#fc0404;">${item.roomName}</td>
                    </c:if>
...
<div class="am-modal am-modal-no-btn" tabindex="-1" id="doc-modal-1">
    <div style="background:#fff;" class="am-modal-dialog">
        <div style="background:#0e90d2; color: #fff" class="am-modal-hd">客房等级操作
                    ...
                    <div class="span4">
                        <div class="row-fluid">
                            <label id="bie" class="labelroomnumber"
                                   style="margin-left: -20px; font-size: 15px;">客房类型名：</label>
                            <form action="" method="post" style="float: left;">
                                <input id="newtxtnameid" type="text" placeholder="客房类型名称"
                                       style="width:100%;height: 26px;margin-top: 11px;">
                            </form>
                        </div>
                    </div>
                    <div>
...
<script type="text/javascript">
    ...
    function newaddfunction() {
        ...
        parent.document.getElementById('Mainid').src = '${ctx}/RoomSet/addGuestRoomLevel.do?txtname=' + txtname;
    }
    ...
    function newdeletefunction() {
        ...
        if (chk_value != "") {
            var flag = window.confirm("注意：您确定要永久删除该信息吗?");
            if (flag) {
                parent.document.getElementById("Mainid").src = '${ctx}/RoomSet/deleteGuestRoomLevel.do?id=' + chk_value;
            }
        } else {
            alert("请选择一条或多条数据进行删除");
        }

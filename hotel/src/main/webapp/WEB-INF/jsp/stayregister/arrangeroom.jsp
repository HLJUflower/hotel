...
<body>
<div class="container" style="height:800px;overflow-x:auto;">
    ...
    <div class="span11" style=" border: solid; border-color: #DDDDDD;">
                ...
                <div class="row-fluid">
                    ...
                    <div class="span3">
                        ...
                        <input id="registerTimeId" name="registerTime" id="date"
                               ...
                               onFocus="WdatePicker({skin:'whyGreen',dateFmt:'yyyy-MM-dd HH:mm:ss',minDate:CurentTime(),maxDate:'#F{\'%y-%M-%d\'}',onpicked:pickedFunc})"
                    ...
                    <div class="span3">
                        ...
                        <select name="billUnitID" style="width:100%;height:27px;">
                            <c:forEach items="${listBillUnit}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==27}">selected="selected"</c:if>>
                                        ${item.attributeDetailsName}
                                </option>
                            </c:forEach>
                        </select>
        ...
        <div class="span12" style="margin-top:-17px;">
            <div class="row-fluid">
                ...
                <button class="btn btn-info btn-small" type="button" onclick="fivefunction()">豪华单人间</button>
        ...
        <div class="span12" id="dynamicid" style="height:400px;overflow-x:auto;">
            <div class="row-fluid" id="div1">
                <c:forEach items="${list}" var="item">
                    <c:if test="${item.roomStateID==1}">
                        <button onclick="suibian(this)"
                                style="width:95px;;height:93px;border: 3px solid #666666;  float:left;margin:2px; background:rgba(19,226,40,0.61);">
                    ...
                    <c:if test="${item.roomStateID==2}">
                        <button onclick="suibian(this)"
                                style="width:95px;;height:93px;border: 3px solid #666666; float:left;margin:2px; background:#e3bfbf;">
                            <input style="display: none;" value="${item.id}"/>
                            <input style="display: none;" value="${item.roomStateID}"/>
                            <label>${item.roomNumber}</label>
                            <label style="margin-top:-5px;">${item.roomName}</label>
                    ...
                    <c:if test="${item.roomStateID==4}">
                        <button onclick="suibian(this)"
                                style="width:95px;;height:93px;border: 3px solid #666666; float:left;margin:2px; background:#63cdcd;">
                            <input style="display: none;" value="${item.id}"/>
                            <input style="display: none;" value="${item.roomStateID}"/>
                            <label>${item.roomNumber}</label>
                            <label style="margin-top:-5px;">${item.roomName}</label>
                    ...
                    <c:if test="${item.roomStateID==5}">
                        <button onclick="suibian(this)"
                                style="width:95px;;height:93px;border: 3px solid #666666; float:left;margin:2px; background:#d0d00e;">
                            <input style="display: none;" value="${item.id}"/>
                            <input style="display: none;" value="${item.roomStateID}"/>
                            <label>${item.roomNumber}</label>
                            <label style="margin-top:-5px;">${item.roomName}</label>
                    ...
                    <c:if test="${item.roomStateID==6}">
                        <button onclick="suibian(this)"
                                style="width:95px;;height:93px;border: 3px solid #666666; float:left;margin:2px; background:#e57044;">
                            <input style="display: none;" value="${item.id}"/>
                            <input style="display: none;" value="${item.roomStateID}"/>
                            <label>${item.roomNumber}</label>
                            <label style="margin-top:-5px;">${item.roomName}</label>
                    ...
                    <c:if test="${item.roomStateID==7}">
                        <button onclick="suibian(this)"
                                style="width:95px;;height:93px;border: 3px solid #666666; float:left;margin:2px; background:#fa0519;">
                            <input style="display: none;" value="${item.id}"/>
                            <input style="display: none;" value="${item.roomStateID}"/>
                            <label>${item.roomNumber}</label>
                            <label style="margin-top:-5px;">${item.roomName}</label>
...
<script type="text/javascript">
    ...
    function tijiao() {
        if (document.getElementById("registerTimeId").value == "") {
            ...
            document.getElementById("registerTimeId").focus();
            return false;
        } else if (document.getElementById("stayNumberId").value.length > 5) {
            alert("你输入的    住店天数/小时   太过长了  请不要超出  5 位长度");
            document.getElementById("stayNumberId").focus();
            return false;
        } else if (document.getElementById("depositId").value.length > 5) {
            alert("你输入的   押金  太过长了  请不要超出  5 位长度  ");
            document.getElementById("depositId").focus();
            return false;
        }
            ...
            success: function (result) {                          // 不出现异常 进行立面方
                for (var key in result) {
                    ...
                    if (item.roomStateID == 1) {
                        var btn = $("<button onclick='suibian(this)' style='width:95px;;height:93px;border: 3px solid #666666; float:left;margin:2px; background:rgba(19,226,40,0.61);'>" +
                            "<input style='display: none;' value=" + item.id + " />" +
                            "<input style='display: none;' value=" + item.roomStateID + " />" +
                            "<label>" + item.roomNumber + "</label>" +
                            "<label style='margin-top:-5px;'>" + item.roomName + "</label>" +

        ...
        <form action="${ctx}/Passenger/add.do" method="post" onsubmit="return verify()">
            ...
            <div class="span12">
                <div class="row-fluid">
                    <div class="span3">
                        <label>姓名：</label>
                        <input id="nameId" name="name" type="text" style="width:97%;height:27px;float:left;"
                        ...
                        <select name="genderID" style="width:100%;height:27px;">
                            <c:forEach items="${listGender}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==31}">selected="selected"</c:if>>
                    ...
                    <div class="span3">
                        <label>出生日期：</label>
                        <input id="birthDateId" name="birthDate" type="text" style="width:100%;height:27px;"
                               placeholder="年-月-日">
                    </div>
                    <div class="span3">
                        <label>民族：</label>
                        <select name="nationID" style="width:100%;height:27px;">
                            <c:forEach items="${listNation}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==33}">selected="selected"</c:if>>
                                        ${item.attributeDetailsName}
                                </option>
                            </c:forEach>
                        </select>
            ...
            <div class="span12">
                <div class="row-fluid">
                    ...
                    <div class="span3">
                        <label>证件有效期：</label>
                        <input id="papersValidityId" name="papersValidity" type="text" style="width:100%;height:27px;"
                    ...
                    <div class="span3">
                        ...
                        <select name="educationDegreeID" style="width:100%;height:27px;">
                            <c:forEach items="${listEducationDegree}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==43}">selected="selected"</c:if>>
                                        ${item.attributeDetailsName}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        ...
                        <select name="passengerLevelID" style="width:100%;height:27px;">
                            <c:forEach items="${listPassengerLevel}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==52}">selected="selected"</c:if>>
                                        ${item.attributeDetailsName}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        ...
                        <select id="papersId" name="papersID" style="width:100%;height:27px;">
                            <c:forEach items="${listPapers}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==37}">selected="selected"</c:if>>
                                        ${item.attributeDetailsName}
                                </option>
                            </c:forEach>
                        </select>
                ...
                <div class="row-fluid">
                    <div class="span3">
                        ...
                        <select name="thingReasonID" style="width:100%;height:27px;">
                            <c:forEach items="${listThingReason}" var="item">
                                <option value="${item.far_id}"
                                        <c:if test="${item.far_id==51}">selected="selected"</c:if>>
                                        ${item.attributeDetailsName}
                                </option>
                            </c:forEach>
                        </select>
                    ...
                    <div class="span3">
                        ...
                        <input id="contactPhoneNumberId" name="contactPhoneNumber" type="text"
                               style="width:97%;height:27px;float: left" placeholder="只能为手机号码" onchange="onchangeOne()">
                        <div id="divPhone" style="float:right;">
                            <label class="yansered" style="margin-top:7px;">*</label>
                        </div>
                    </div>
                </div>
    ...
    function onchangeOne() {
        ...
        if (document.getElementById("papersNumberId").value != "") {
            ...
            if (document.getElementById("papersId").value == 37) {
                if (!document.getElementById("papersNumberId").value.trim().match("^[1-9][0-9]{5}(19|20)[0-9]{2}((01|03|05|07|08|10|12)(0[1-9]|[1-2][0-9]|31)|(04|06|09|11)(0[1-9]|[1-2][0-9]|30)|02(0[1-9]|[1-2][0-9]))[0-9]{3}([0-9]|x|X)$")) {
                    document.getElementById("papersNumberId").focus();
        ...
        //联系电话  contactPhoneNumberId
        if (document.getElementById("contactPhoneNumberId").value != "") {
            document.getElementById("divPhone").style.display = "none";
            if (!document.getElementById("contactPhoneNumberId").value.trim().match("^(?:\\+?86)?1(?:3\\d{3}|5[^4\\D]\\d{2}|8\\d{3}|7(?:[35678]\\d{2}|4(?:0\\d|1[0-2]|9\\d))|9[189]\\d{2}|66\\d{2})\\d{6}$")) {
                document.getElementById("contactPhoneNumberId").focus();
                document.getElementById("contactPhoneNumberId").value = "";
                alert("你输入的手机号码有误，请重新输入！");
            }
        }else{
            document.getElementById("divPhone").style.display = "block"; //显示
        }
    }
